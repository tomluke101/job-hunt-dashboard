...
15e. NAMED-PRODUCT INACCURACY — when the candidate's profile names a specific product/system/initiative (e.g. "AI-powered sales training", "supplier performance tracker", "Airtable ERP"), the letter must use that exact name or a close synonym, not a genericised version. Banned shape: profile says "AI-powered sales training" → letter says "AI-powered delivery tools" (genericised). Profile says "supplier performance tracker" → letter says "performance management system" (genericised). FIX: use the specific name from the profile.
  ...
  try {
    ...
    const result = await callAI({
      ...
      systemPrompt:
        ...
        "STRUCTURE: 1-2 sentences naming the target role/function and what about their current work draws them to it. Then 2-4 sentences naming SPECIFIC concrete achievements from their profile that demonstrate transferable capabilities. End with at most one sentence connecting their direction to the target company or role (only if a genuine concrete connection exists — never generic flattery). " +
        "FACT DISCIPLINE — USE ONLY FACTS FROM THE PROFILE. Never invent achievements. Never invent abstract qualities the candidate hasn't claimed ('commercial instincts', 'solution-shaping', 'business acumen', 'strategic vision', 'product intuition' — banned unless explicitly in profile). Never inflate the audience: 'senior stakeholders' is banned for small-business roles unless profile establishes it. Never stretch the meaning of an achievement to fit the target — e.g. 'conversion rate optimisation tests on product samples' must NOT be reframed as 'understanding what moves a prospect toward a decision'; describe it accurately. If the candidate's draft is provided, preserve their voice and intent but expand with profile-grounded specifics. " +
        "ROLE-AS-SUBJECT BAN — UNIVERSAL SHAPE: any sentence whose subject is the role / company / company's focus / company's commitment / work-at-company, listing JD items and claiming alignment, is banned regardless of verb. Specifically banned: '[Company]'s focus on X is exactly what I want', '[Company]'s commitment to Y aligns with my approach to Z', 'X mirrors / maps to / aligns with / matches what this role demands / requires / calls for'. The subject of every sentence must be the candidate or the candidate's work, not the role/company. " +
        "SELF-CHARACTERISING SUMMARY BAN: never write sentences like '[abstract quality] is embedded in how I work', 'X has shaped how I approach Y', 'that experience gave me a grounding in Z', 'my approach is built on X', 'I bring a strong sense of Y'. State concrete facts and stop — let the reader infer the qualities. " +
        "BANNED OPENERS / FRAMINGS: 'I'm transitioning from X to Y', 'I'm pivoting from X', 'while my background is in X', 'although I don't have direct experience in X', any apologetic framing for the pivot. Frame as 'the work I want to do is X' or 'I want to move into X' followed by concrete grounding. Never explain the gap. " +
        "BANNED PHRASES: 'passionate about', 'driven by', 'thrive in', 'best-in-class', 'world-class', 'results-oriented', 'proven track record', 'demonstrated ability to', 'strategic mindset', 'cross-functional excellence', 'meaningful product set', 'commercial instincts', 'solution-shaping', 'data-driven decision-making' (unless quoting the JD), 'meaningful impact', 'genuinely changes', 'consequential work'. Sound like a real person, not a recruiter template. " +
