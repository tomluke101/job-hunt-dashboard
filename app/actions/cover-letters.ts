...
[R1] COMPANY-NAMED-IN-BODY (only when target company is KNOWN): if the user provided a target company name OR the JD names a target company, the company's name MUST appear at least once in the letter body. EXCEPTION: if no target company is identifiable, do not enforce this rule — instead enforce [R1B].
...
(1) COMPANY-NAMED-IN-BODY — REQUIRED ONLY WHEN THE TARGET COMPANY IS KNOWN. If the user provided a company name OR the JD clearly names the target company, that name MUST appear at least once in the letter body (anywhere except sentence 1 of P1). Natural placements: inside a JD-integration sentence ("the [function] at [Company] handles..."), as a mid-sentence reference in P2 or P3, or in the closing. CRITICAL EXCEPTION: if no target company is provided AND the JD does not name the target company, do NOT invent one and do NOT default to the candidate's CURRENT employer name. The candidate's current employer is the place they currently work, not the place they're applying to — using it in the JD-integration sentence creates nonsense ("the X function at [my own employer]" reads as if the candidate is describing their own role's function, not the role they're applying to). When no target company is known, the JD-integration sentence must use a generic reference ("this role", "the role described") and the closing must use "this role" or "the [named function from JD] role" — never reference any company.
