...
- WHY-THIS-COMPANY MOMENT — ACTIVELY HUNT FOR ONE: before defaulting to "skip the why-this-company moment", you MUST scan the JD and the candidate's profile for an evidenced hook. The JD itself almost always contains hookable material. Look specifically for: (a) named systems / processes / acronyms / methodologies (e.g. SSDS, CMMS, SPEEDY, lean manufacturing), (b) named teams / functions (e.g. Release and Follow-Up, MP&L), (c) listed benefits or development schemes that signal company values (e.g. funded learning schemes, apprenticeship programmes, employee resource groups), (d) explicit operational priorities the JD highlights (e.g. "minimising production downtime", "running-change process", "supplier penalty systems"). For each candidate hook, check the candidate's profile / skills / CV for genuine evidence to pair it with. If a hook + evidence pair exists, USE IT — either as a brief integrated sentence within P2 or P3, or (rarely) as a dedicated short why-this-company paragraph. Do NOT default to "no why-this-company content" if a hook + evidence pair is available; that is leaving easy fit-signal on the table.
  Examples of evidenced hook + evidence pairs (do not copy verbatim):
    - JD names "funded learning scheme" + candidate has self-taught technical systems and a top degree → "JLR's funded development scheme is part of why this role appeals — I've self-taught the Airtable ERP and supplier tracking systems I built at Grain and Frame, and I'd value an employer that actively backs continued development."
    - JD names "SSDS running-change process" + candidate has handled product transitions / phase-outs → "Managing the running-out of legacy SKUs alongside new product launches is something I've done at Grain and Frame, and the SSDS running-change process at JLR is exactly the structured version of that work."
    - JD names "drive supplier performance through SPEEDY" + candidate built supplier performance tracking → "Driving supplier accountability through structured tools is the work I built into our supplier performance tracking system, and SPEEDY is the same discipline at scale."
  These integrated sentences should be 1-2 sentences max — do NOT pad them into a full paragraph unless there is genuinely a paragraph's worth of evidenced material.
