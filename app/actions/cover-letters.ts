...
14. JD-META-COMMENTARY — sentences that tell the JD's reader what their own JD says are banned. The hiring manager wrote (or has read) the JD; they do not need it summarised back to them. Banned constructions: "[X] is listed as a significant advantage in the JLR role", "[X] is mentioned in the JD as essential", "[X] is something the role specifies", "as the JD notes, [X]", "you mention that [X]", "you've outlined [X] as a key responsibility", "the role asks for [X], and [Y]", "as outlined in the job description", "The [Job Title] role at [Company] [calls for / requires / asks for / specifies / centres on / is built around] [list of JD items]" (this opens a paragraph by listing the JD's requirements back at the reader, with redundant role+company naming on top), "The [X] work at [Company] [maps closely to / matches / mirrors / aligns with] what I do [now / day-to-day]: [list of JD items]" (same family — sentence subject is the role/work at the company, then lists JD items, then claims they're what the candidate does), "This role demands [list of JD items]", "[Company]'s [function] is responsible for [list of JD items]" used as a paragraph opener that then pivots to the candidate's parallel work. These all narrate the JD back at its author. FIX: drop the meta-commentary entirely. Open the paragraph with the candidate's own concrete work, OR use a clean parallel-work pattern: "At [current employer], I do the same work this role calls for: [list]" or "A large part of my current role mirrors what [named JD function] handles: [list]." Never list JD requirements in a sentence whose subject is the role/role title/company.

14b. CROSS-PARAGRAPH ATTRIBUTION CONSISTENCY — read the entire letter before finalising and check that NO synthesised claim contradicts the candidate's actual profile attribution. Every concrete activity statement ("I introduced X", "I built Y", "I led Z") MUST be attributable to a specific employer that matches the candidate's Work History and Skills tags. If the candidate has the SAME or SIMILAR work tagged to TWO employers (e.g. AI tools researched at one employer and implemented at another), be precise: do not write a general "I introduced AI tools" claim that's ambiguous about which employer or that conflates the two. Either:
   (a) State each one separately in its own employer's paragraph with its actual scope (e.g. "At Grain and Frame I researched how the team could use AI to improve efficiency" + "At Siemens, I collaborated with my manager on the introduction of AI-powered sales training"), OR
   (b) Pick the stronger one and only mention that in the relevant paragraph.
   NEVER write the same/similar claim in two paragraphs about two different employers, with different attribution shapes ("I introduced" vs "I collaborated on the introduction of") — recruiters notice contradictions and the letter loses credibility.
