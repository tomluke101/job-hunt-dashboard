...
import { setMasterExclusions } from "@/app/actions/cv-tailoring";

interface Props {
  initial: string[];
  // True if the user has saved a Master row at all. Without one, exclusions
  // can't attach (we don't create empty Master rows just to hold exclusions).
  hasSavedMaster: boolean;
}

export default function MasterExclusionsSection({ initial, hasSavedMaster }: Props) {
  ...
  function handleSave() {
    ...
    startSave(async () => {
      const r = await setMasterExclusions(items);
      if (r.error) {
        setError(r.error);
  ...
  return (
    <div className="space-y-3">
      <p className="text-xs text-slate-500 leading-relaxed">
        Phrases or topics the AI should NEVER include in your Profile, no matter how relevant the JD looks. Useful when the AI keeps reaching for something you&apos;ve deliberately left out — a system you don&apos;t want highlighted, an old role, a specific claim.
      </p>
      ...
      {!hasSavedMaster && (
        <div className="rounded-lg border border-amber-200 bg-amber-50 p-3 text-xs text-amber-900 flex items-start gap-1.5">
          ...
          <span>Save your Master Profile above first — exclusions attach to it.</span>
        </div>
      )}

      {/* Existing exclusions as chips */}
      {items.length > 0 && (
      ...
      <div className="flex items-center gap-2">
        ...
        <button
          onClick={handleAdd}
          disabled={!hasSavedMaster || isSaving || !draft.trim()}
      ...
      <div className="flex items-center justify-between gap-2 flex-wrap">
        ...
        <button
          onClick={handleSave}
          disabled={!hasSavedMaster || isSaving || !isDirty}
      ...
      {error && (
        <div className={`rounded-lg border p-3 text-xs flex items-start gap-1.5 ${
          needsMigration
